...
Game_Controller::Game_Controller(int board_size, char board_location)
{
	this->lock = false;
	this->handicap = false;
...
bool Game_Controller::set_stone(Player*& player, Point p)
{
...
bool Game_Controller::is_suicide(Player*& player, Point p)
{
...
int Game_Controller::can_beat(Player*& player, Point p)
{
	int beated = 0;
...
bool Game_Controller::has_even_one_breathe(Point p, char search)
{
	Point possibilities[4] =
	{
		{p.x - 1, p.y}, {p.x + 1, p.y},
...
bool Game_Controller::is_already_checked(Point point)
{
...
void Game_Controller::remove_points()
{
	for (int i = 0; i < how_many_checked; i++)
		board->set_value_by_pos(checked[i], field_type::INTERIOR);
}

void Game_Controller::clear_checked_list()
{
	for (int i = 0; i < how_many_checked; i++)
		checked[i] = { -1, -1 };
	how_many_checked = 0;
...
void Game_Controller::setup_points()
{
...
bool Game_Controller::save(const char* file_name)
{
	FILE* file;
...
bool Game_Controller::load(const char* file_name)
{
	FILE* file;
...
void Game_Controller::handle_input(int input)
{
	switch (input)
	{
		case 0:
